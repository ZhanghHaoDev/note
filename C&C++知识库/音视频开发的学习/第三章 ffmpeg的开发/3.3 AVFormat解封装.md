...
int main() {
    std::string test_file = "/home/zh/ffmpeg/test_mp4.mp4";

    AVFormatContext *pFormatCtx = avformat_alloc_context();
    if (avformat_open_input(&pFormatCtx, test_file.c_str(), NULL, NULL) != 0) {
    ...
    if (audio_stream_index == -1) {
        std::cerr << "Error: Couldn't find audio stream." << std::endl;
    ...
    if (!audio_codec) {
        std::cerr << "Error: Couldn't find audio codec." << std::endl;
        avformat_close_input(&pFormatCtx);
        avformat_free_context(pFormatCtx);
        return -1;
    }

    std::string output_file = "audio." + std::string(audio_codec->name);

    AVFormatContext *audio_output_ctx = nullptr;
    avformat_alloc_output_context2(&audio_output_ctx, nullptr, nullptr, output_file.c_str());
    if (!audio_output_ctx) {
        std::cerr << "Error: Couldn't create output context." << std::endl;
        avformat_close_input(&pFormatCtx);
        avformat_free_context(pFormatCtx);
        return -1;
    }

    AVStream *out_stream = avformat_new_stream(audio_output_ctx, nullptr);
    if (!out_stream) {
        std::cerr << "Error: Couldn't create output stream." << std::endl;
        avformat_close_input(&pFormatCtx);
        avformat_free_context(pFormatCtx);
    ...
    if (avcodec_parameters_copy(out_stream->codecpar, audio_codecpar) < 0) {
        std::cerr << "Error: Couldn't copy codec parameters." << std::endl;
        avformat_close_input(&pFormatCtx);
        avformat_free_context(pFormatCtx);
        avformat_free_context(audio_output_ctx);
        return -1;
    ...
    if (!(audio_output_ctx->oformat->flags & AVFMT_NOFILE)) {
        if (avio_open(&audio_output_ctx->pb, output_file.c_str(), AVIO_FLAG_WRITE) < 0) {
    ...
    if (avformat_write_header(audio_output_ctx, nullptr) < 0) {
        std::cerr << "Error: Couldn't write header." << std::endl;
        avformat_close_input(&pFormatCtx);
        avformat_free_context(pFormatCtx);
        avformat_free_context(audio_output_ctx);
        return -1;
    ...
    while (av_read_frame(pFormatCtx, &packet) >= 0) {
        if (packet.stream_index == audio_stream_index) {
            ...
            av_write_frame(audio_output_ctx, &packet);
        }
